...
class DownloadService : BaseService() {
        ...
        when (intent?.action) {
            ...
            IntentAction.play -> {
                ...
                if (completeDownloads.contains(id)) {
                    ...
                    ToastUtil.showToast("未完成,下载的文件夹Download")
                }
            }
            IntentAction.stop -> {
    ...
    private fun startDownload(url: String?, fileName: String?) {
        ...
        kotlin.runCatching {
            ...
            when (it) {
                is SecurityException -> ToastUtil.showToast("下载出错,没有存储权限")
                else -> ToastUtil.showToast("下载出错,${it.localizedMessage}")
            }
        }
    }
    ...
    private fun successDownload(downloadId: Long) {
        if (!completeDownloads.contains(downloadId)) {
            ...
            if (fileName?.endsWith(".apk") == true) {
                ...
                ToastUtil.showToast("$fileName ${getString(R.string.download_success)}")
            }
        }
    }
