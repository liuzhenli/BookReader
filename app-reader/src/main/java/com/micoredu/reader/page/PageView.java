...
import com.micoredu.reader.animation.ScrollPageAnim;
import com.micoredu.reader.animation.SimulationPageAnim;
import com.micoredu.reader.animation.SlidePageAnim;
...
import static com.liuzhenli.common.utils.ScreenUtils.getDisplayMetrics;
...
public class PageView extends View implements PageAnimation.OnPageChangeListener {
    ...
    private List<TxtLine> mLinesData = null;
        ...
        mLongPressRunnable = new Runnable() {
            ...
            public void run() {
                ...
                if (mStartX > 0 && mStartY > 0) {
                    ...
                    isLongPress = true;
                    //找到长按的点
                    TxtChar p = mPageLoader.detectPressTxtChar(mStartX, mStartY);
                    //设置开始位置字符
                    firstSelectTxtChar = p;
                    //设置结束位置字符
                    lastSelectTxtChar = p;
                    //设置模式为长按选择
                    selectMode = SelectMode.PressSelectText;
                    //响应长按事件，供上层调用
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        switch (event.getAction()) {
            ...
            case MotionEvent.ACTION_UP:
                ...
                //长安选择删除选中状态
                if (firstSelectTxtChar == null || isMove) {
                    mPageAnim.onTouchEvent(event);
                } else {
                    if (!isLongPress) {
                        //释放了
    ...
    public PageLoader getPageLoader(BaseReaderActivity activity, @NotNull BookShelfBean bookShelfBean, PageLoader.Callback callback) {
