...
import com.micoredu.reader.helper.ChapterContentHelp;
...
import static com.micoredu.reader.utils.ReaderConfig.DEFAULT_MARGIN_WIDTH;
...
public abstract class PageLoader {
    ...
    public void prepareDisplay(int w, int h) {
        ...
        mMarginTop = hideStatusBar ?
                ScreenUtils.dpToPx(readBookControl.getTipPaddingTop() + readBookControl.getPaddingTop() + ReaderConfig.DEFAULT_MARGIN_HEIGHT)
                : ScreenUtils.dpToPx(readBookControl.getPaddingTop());
        mMarginBottom = ScreenUtils.dpToPx(readBookControl.getTipPaddingBottom() + readBookControl.getPaddingBottom() + ReaderConfig.DEFAULT_MARGIN_HEIGHT);
        ...
        tipMarginTop = ScreenUtils.dpToPx(readBookControl.getTipPaddingTop() + ReaderConfig.DEFAULT_MARGIN_HEIGHT);
        tipMarginBottom = ScreenUtils.dpToPx(readBookControl.getTipPaddingBottom() + ReaderConfig.DEFAULT_MARGIN_HEIGHT);
    ...
    private void initPaint() {
        ...
        mBatteryPaint.setTextSize(ScreenUtils.spToPx(ReaderConfig.DEFAULT_TIP_SIZE - 3));
    ...
    private void initPageStyle() {
        ...
        mTextEndPaint.setColor(readBookControl.getTextColor());
        mTipPaint.setAlpha(ReaderConfig.TIP_ALPHA);
        mBatteryPaint.setAlpha(ReaderConfig.TIP_ALPHA);
        mTextEndPaint.setAlpha(ReaderConfig.TIP_ALPHA);
    }
    ...
    public boolean skipNextChapter() {
        if (mCurChapterPos + 1 >= mBookShelfBean.getChapterListSize()) {
            return false;
        }
    ...
    public String getContent() {
        ...
        if (curChapter().txtChapter.getPageSize() == 0) {
            return null;
        }
    ...
    private void upPage() {
        if (mPageMode != PageAnimation.Mode.SCROLL) {
            ...
            if (mCurPagePos < curChapter().txtChapter.getPageSize() - 1 || curChapter().txtChapter.getPosition() < mCallback.getChapterList().size() - 1) {
                mPageView.drawPage(1);
            }
        }
    }
    ...
    public void pagingEnd(PageAnimation.Direction direction) {
        ...
        switch (direction) {
            case NEXT:
                ...
                } else if (mCurChapterPos < mBookShelfBean.getChapterListSize() - 1) {
                    mCurChapterPos = mCurChapterPos + 1;
                    mCurPagePos = 0;
                    Collections.swap(chapterContainers, 0, 1);
                    Collections.swap(chapterContainers, 1, 2);
        ...
        mBookShelfBean.setDurChapter(mCurChapterPos);
        mBookShelfBean.setDurChapterPage(mCurPagePos);
        mCallback.onPageChange(mCurChapterPos, getCurPagePos(), resetReadAloud);
        resetReadAloud = true;
    }
     ...
     */
    public synchronized void drawPage(Bitmap bitmap, int pageOnCur) {
        ...
        if (pageOnCur == 0) { //当前页
            ...
            if (mCurPagePos + 1 < curChapter().txtChapter.getPageSize()) {
                ...
                if (mCurChapterPos + 1 >= mCallback.getChapterList().size()) {
                    txtChapter = new TxtChapter(mCurChapterPos + 1);
                    txtChapter.setStatus(TxtChapter.Status.ERROR);
    ...
    private synchronized void drawBackground(final Canvas canvas, TxtChapter txtChapter, TxtPage txtPage) {
        ...
        if (!mCallback.getChapterList().isEmpty()) {
            ...
            title = ChapterContentHelp.getInstance().replaceContent(mBookShelfBean.getBookInfoBean().getName(), mBookShelfBean.getTag(), title, mBookShelfBean.getReplaceEnable());
            //页码
            ...
            String progressStr = (txtChapter.getStatus() != TxtChapter.Status.FINISH) ? ""
                    : BookshelfHelper.getReadProgress(mCurChapterPos, mBookShelfBean.getChapterListSize(), mCurPagePos, curChapter().txtChapter.getPageSize());
     ...
     * 绘制内容-滚动
     */
    public void drawContent(final Canvas canvas, float offset) {

        if (offset > ReaderConfig.MAX_SCROLL_OFFSET) {
            offset = ReaderConfig.MAX_SCROLL_OFFSET;
        } else if (offset < 0 - ReaderConfig.MAX_SCROLL_OFFSET) {
            offset = -ReaderConfig.MAX_SCROLL_OFFSET;
        }
        ...
        while (top < totalHeight) {
            ...
            if (chapter == null || chapterPos - mCurChapterPos > 1) {
                break;
            }
            ...
            if (pagePos == chapter.getPageSize() - 1) {
                String sign = "\u23af \u23af";
                if (chapterPos == mBookShelfBean.getChapterListSize() - 1) {
    ...
    public boolean hasNext(int pageOnCur) {
        ...
        return mCurChapterPos + 1 < mBookShelfBean.getChapterListSize();
    }
    ...
    void parseCurChapter() {
        if (curChapter().txtChapter.getStatus() != TxtChapter.Status.FINISH) {
            Single.create((SingleOnSubscribe<TxtChapter>) e -> {
                ...
                TxtChapter txtChapter = chapterProvider.dealLoadPageList(mCallback.getChapterList().get(mCurChapterPos), mPageView.isPrepare());
    ...
    void parsePrevChapter() {
        ...
        Single.create((SingleOnSubscribe<TxtChapter>) e -> {
            ChapterProvider chapterProvider = new ChapterProvider(this);
            TxtChapter txtChapter = chapterProvider.dealLoadPageList(mCallback.getChapterList().get(prevChapterPos), mPageView.isPrepare());
            e.onSuccess(txtChapter);
        })
                .compose(RxUtil::toSimpleSingle)
                .subscribe(new SingleObserver<TxtChapter>() {
    ...
    void parseNextChapter() {
        ...
        if (nextChapterPos >= mCallback.getChapterList().size()) {
            nextChapter().txtChapter = null;
            return;
        }
        ...
        Single.create((SingleOnSubscribe<TxtChapter>) e -> {
            ChapterProvider chapterProvider = new ChapterProvider(this);
            TxtChapter txtChapter = chapterProvider.dealLoadPageList(mCallback.getChapterList().get(nextChapterPos), mPageView.isPrepare());
            e.onSuccess(txtChapter);
        })
                .compose(RxUtil::toSimpleSingle)
                .subscribe(new SingleObserver<TxtChapter>() {
    ...
    private void chapterChangeCallback() {
        if (mCallback != null) {
            ...
            mCallback.onChapterChange(mCurChapterPos);
            mCallback.onPageCountChange(curChapter().txtChapter != null ? curChapter().txtChapter.getPageSize() : 0);
        }
    }
     ...
     */
    public TxtChar detectPressTxtChar(float down_X2, float down_Y2) {
        TxtPage txtPage = curChapter().txtChapter.getPage(mCurPagePos);
        if (txtPage == null) {
            return null;
        }
