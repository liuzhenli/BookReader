...
import com.micoredu.reader.bean.DownloadChapterBean;
import com.micoredu.reader.helper.BookshelfHelper;
import com.micoredu.reader.impl.IDownloadTask;
...
public abstract class DownloadTaskImpl implements IDownloadTask {
    ...
    protected DownloadTaskImpl(int id, DownloadBookBean downloadBook) {
        ...
        Observable.create((ObservableOnSubscribe<DownloadBookBean>) emitter -> {
            List<BookChapterBean> chapterList = BookshelfHelper.getChapterList(downloadBook.getNoteUrl());
            if (!chapterList.isEmpty()) {
                for (int i = downloadBook.getStart(); i <= downloadBook.getEnd(); i++) {
                    ...
                    if (!BookshelfHelper.isChapterCached(chapter.getBookName(), chapter.getTag(), chapter, false)) {
    ...
    private Observable<DownloadChapterBean> getDownloadingChapter() {
        return Observable.create(emitter -> {
            ...
            for (DownloadChapterBean data : temp) {
                boolean cached = BookshelfHelper.isChapterCached(data.getBookName(), data.getTag(), data, false);
                if (cached) {
