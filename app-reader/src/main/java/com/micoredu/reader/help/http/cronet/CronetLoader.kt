package com.micoredu.reader.help.http.cronet
...
import com.liuzhenli.common.utils.AppConst.Cronet_Version
...
object CronetLoader : CronetEngine.Builder.LibraryLoader() {
    ...
    private const val soVersion = Cronet_Version
    ...
    init {
        ...
        LogUtils.e(javaClass.simpleName, "soName+:$soName")
        LogUtils.e(javaClass.simpleName, "destSuccessFile:$soFile")
        LogUtils.e(javaClass.simpleName, "tempFile:$downloadFile")
        LogUtils.e(javaClass.simpleName, "soUrl:$soUrl")
    }
    ...
    fun preDownload() {
        ...
        Coroutine.async {
            ...
            if (soFile.exists() && md5 == getFileMD5(soFile)) {
                LogUtils.e(javaClass.simpleName, "So 库已存在")
            } else {
                download(soUrl, md5, downloadFile, soFile)
            }
            LogUtils.e(javaClass.simpleName, soName)
        }
    }
    ...
    override fun loadLibrary(libName: String) {
        LogUtils.e(javaClass.simpleName, "libName:$libName")
        ...
        try {
            ...
            //首先调用系统行为进行加载
            System.loadLibrary(libName)
            LogUtils.e(javaClass.simpleName, "load from system")
        } catch (e: Throwable) {
            //如果找不到，则从远程下载
            //删除历史文件
            deleteHistoryFile(Objects.requireNonNull(soFile.parentFile), soFile)
            //md5 = getUrlMd5(md5Url)
            LogUtils.e(javaClass.simpleName, "soMD5:$md5")
            ...
            if (soFile.exists()) {
                ...
                if (fileMD5 != null && fileMD5.equals(md5, ignoreCase = true)) {
                    ...
                    LogUtils.e(javaClass.simpleName, "load from:$soFile")
                    return
                }
                //md5不一样则删除
                soFile.delete()
            ...
            LogUtils.e(javaClass.simpleName, "time:" + (System.currentTimeMillis() - start))
        }
    }
        ...
        if (files != null && files.isNotEmpty()) {
            for (f in files) {
                if (f.exists() && (currentFile == null || f.absolutePath != currentFile.absolutePath)) {
                    val delete = f.delete()
                    LogUtils.e(javaClass.simpleName, "delete file: $f result: $delete")
                    if (!delete) {
                        f.deleteOnExit()
                    }
                }
    ...
    private fun download(
        ...
        Coroutine.async {
            ...
            LogUtils.e(javaClass.simpleName, "download result:$result")
            //文件md5再次校验
            ...
            LogUtils.e(javaClass.simpleName, "download success, copy to $destSuccessFile")
            //下载成功拷贝文件
