package com.micoredu.reader.help.storage
...
import com.liuzhenli.common.utils.getFile
...
import com.micoredu.reader.help.ReplaceAnalyzer
                    ...
                    "myBookShelf.json" ->
                        kotlin.runCatching {
                            doc.uri.readText(context).let { json ->
                                ...
                                ToastUtil.showToast("成功导入书籍${importCount}")
                            }
                        }.onFailure {
                            ToastUtil.showToast("导入书籍失败\n${it.localizedMessage}")
                        }
                    "myBookSource.json" ->
                        kotlin.runCatching {
                            doc.uri.readText(context).let { json ->
                                val importCount = importOldSource(json)
                                ToastUtil.showToast("成功导入书源${importCount}")
                            }
                        }.onFailure {
                            ToastUtil.showToast("导入源失败\n${it.localizedMessage}")
                        }
                    "myBookReplaceRule.json" ->
                        kotlin.runCatching {
                            doc.uri.readText(context).let { json ->
                                val importCount = importOldReplaceRule(json)
                                ToastUtil.showToast("成功导入替换规则${importCount}")
                            }
                        }.onFailure {
                            ToastUtil.showToast("导入替换规则失败\n${it.localizedMessage}")
                        }
                }
            }
        } else {
    ...
    private fun fromOldBooks(json: String): List<Book> {
        ...
        for (item in items) {
            ...
            if (book.bookUrl in existingBooks) {
                LogUtils.e(javaClass.name, "Found existing book: " + book.name)
                continue
            }
